...
fn main() -> std::io::Result<()> {
    let mut targets: Vec<String> = env::args().collect();
    
    if targets.len() > 1 {
        targets.remove(0);
    } else {
        println!("\nOops... it appears that you forgot to supply any arguments.\n\nThe proper format for calling this utility is 'rmdear <directory(s)>'.");
        return Ok(()) // I've already done an error message. Also, for some reason Rust won't allow me to return anything else :)
    }
